...
class RNN_model(nn.Module):
    def __init__(self, input_size, hidden_size, seq_len, num_layers, bidirectional, rnn_type, dropout, layer_norm, device='cuda'):
        ...
        self.fc = nn.Linear(self.num_directions * self.hidden_size, 1)                  # 마지막 hidden state만 사용
        # self.fc = nn.Linear(self.num_directions * self.hidden_size * self.seq_len, 1) # 모든 hidden state 사용
    ...
    def forward(self, x):
        ...
        out = self.fc(out[:,-1,:])                      # 마지막 hidden state 사용
        # out = self.fc(out.reshape(out.shape[0],-1))   # 모든 hidden state 사용
...
class RNN_Attention(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, bidirectional, rnn_type, dropout, layer_norm, device='cuda'):
        ...
        # Attention module활용
        self.attn = Attention(self.hidden_size * self.num_directions, device)
