"""
Inherited from the cod correspoding to [Zerveas et al. 2021]
"""
...
def main(config):
    ...
    if config["task"] == "classification":
        ...
        validation_method = (
            "ShuffleSplit" if config["shuffle"] == "True" else "NormalSplit"
        )
        labels = None
    ...
    if not os.path.exists(
        os.path.join(save_dir, f"train_{str(config['shuffle'])}_df.csv")
    ):
    ...
    if not os.path.join(save_dir, f"valid_{str(config['shuffle'])}_df.csv"):
        ...
        valid_df.to_csv(
            os.path.join(save_dir, f"valid_{str(config['shuffle'])}_df.csv"),
            encoding="cp949",
            index=True,
        )
