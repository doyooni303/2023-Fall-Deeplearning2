...
def split_dataset(data_indices, 
    ...
    datasplitter.split_validation(validation_ratio=validation_ratio, n_splits=n_splits, random_state=random_seed)
...
class DataSplitter(object):
    ...
    def factory(split_type, *args, **kwargs):
        ...
        elif split_type == 'NormalSplit':
            return NormalSplitter(*args,**kwargs)
        else:
...
class ShuffleSplitter(DataSplitter):
    ...
    def split_validation(self, n_splits, validation_ratio, random_state=1337):
        ...
        splitter = model_selection.ShuffleSplit(n_splits=n_splits, 
                                                test_size=validation_ratio,
                                                random_state=random_state)
        # get local indices, i.e. indices in [0, len(train_val_labels)), per fold
...
class NormalSplitter(DataSplitter):
    ...
    def split_testset(self, test_ratio, random_state=1337):
        ...
        self.train_val_indices, self.test_indices = model_selection.train_test_split(self.data_indices,
        ...
        # self.test_indices = self.data_indices[test_indices]
        if self.data_labels is not None:
            self.train_val_labels = self.data_labels[self.train_val_indices]
            self.test_labels = self.data_labels[self.test_indices]

        return

    def split_validation(self, validation_ratio, n_splits = None, random_state=1337):
        """
        Input:
            validation_ratio: ratio of validation set with respect to the entire dataset. Should result in an absolute number of
                samples which is greater or equal to the number of classes
        ...
        self.train_indices, self.val_indices = model_selection.train_test_split(self.train_val_indices,
                                                                       test_size=validation_ratio,
                                                                       shuffle = False,
                                                                       random_state=random_state)
