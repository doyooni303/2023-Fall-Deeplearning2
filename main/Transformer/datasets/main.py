...
def main(config):
    ...
    if config['task'] == 'classification':
        ...
        validation_method = 'ShuffleSplit' if config['shuffle']=='True' else 'NormalSplit'
        labels = None
    ...
    if not os.path.exists(os.path.join(save_dir,f"train_{str(config['shuffle'])}_df.csv")):
        train_df = my_data.feature_df.loc[train_indices]
        train_df['labels'] = my_data.labels_df.loc[train_indices]
        train_df.to_csv(os.path.join(save_dir,f"train_{str(config['shuffle'])}_df.csv"),
                                                    encoding = 'cp949',
                                                    index = True,)
    if not os.path.join(save_dir,f"valid_{str(config['shuffle'])}_df.csv"):
        valid_df = val_data.feature_df.loc[val_indices]
        valid_df['labels'] = val_data.labels_df.loc[val_indices]
        valid_df.to_csv(os.path.join(save_dir,f"valid_{str(config['shuffle'])}_df.csv"),
                                                    encoding = 'cp949',
                                                    index = True,)
    if not os.path.join(save_dir,"test_df.csv"):
        test_df = test_data.feature_df.loc[test_indices]
        test_df['labels'] = test_data.labels_df.loc[test_indices]
        test_df.to_csv(os.path.join(save_dir,"test_df.csv"),
                                                    encoding = 'cp949',
                                                    index = True,)
